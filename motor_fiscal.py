...
def gerar_excel_final(df_ent, df_sai):
    ...
    # Correção do Erro: Garante que o download_button receba um arquivo real
    ...
    def auditoria_final(row):
        ...
        if tem_entradas:
            ...
            st_entrada = "⚠️ XML Entrada não carregado"
        ...
        elif "CST" in res and comp_num == 0: acao = "Cc-e"
        else: acao = "Complemento/Estorno"

        cce = "Sim" if acao == "Cc-e" else "Não"
        
        return pd.Series([st_entrada, res, format_brl(row['VLR-ICMS']), format_brl(row['BC-ICMS'] * aliq_esp / 100 if aliq_esp > 0 else 0), acao, format_brl(comp_num), cce])

    # Colunas com a ST na Entrada em destaque
